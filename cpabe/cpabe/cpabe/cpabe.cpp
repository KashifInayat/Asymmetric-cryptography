...
int find_index(int Access[],int i) //initalize the function find_index() that takes two attributes the access structure and a integer value.
{
	int n,j,k;
	j=k=0;
	while (k<i)
...
int main()
{
	...
	time_t seed;            // The time_t datatype is a data type in the ISO C library defined for storing system time values. Such values
							//	are returned from the standard time() library function
	                        //we use get a seed for our random number.
	int i,j,k,l,n,m,d,nS;      // iinteger variables  later used in the code.
	Big **LSSS;				// get a handler for the LSSS matrix a pointer for LSSSS MATRIX.
	int *attr;                  // pointer for the attributes
	Big det,order=pfc.order();  // get pairing-friendly group order

	Big alpha,a,M,CT,s,t;         //declare the variable like alpha, a , message cihpertext and random variables s and t of type big.
	G2 g2,g2a,*h,CD;             //decalre variables for the generators of group G2 and a pointer *h
	G1 g1,g1a,K,L,MSK;			//decalre variables for the generators of group G1 the mastersecret key and variables Land k.
	GT eg2g1a,mask;               // variable of type GT for the mapping of g1g2a and a mask variable 

	time(&seed);       // initialise (insecure!) random numbers
    irand((long)seed);//  creat random number of long type from the seed value.

// Setup

	pfc.random(alpha);   //create a random value aplha in pairing friendly curve 
	pfc.random(a);	    //create a random value a in pairing friendly curve 
	pfc.random(g1);		//create a random value g1 in pairing friendly curve 
	pfc.random(g2);     ////create a random value g2 in pairing friendly curve 
	pfc.precomp_for_mult(g1);  // g1 is fixed, so precompute on it
	pfc.precomp_for_mult(g2);  // g2 is fixed, so precompute on it

	eg2g1a=pfc.power(pfc.pairing(g2,g1),alpha);//  computing e(g1,g2)^alpha
	pfc.precomp_for_power(eg2g1a);

	g1a=pfc.mult(g1,a);          // computing (g1)^a
	g2a=pfc.mult(g2,a);          //computing (g2)^a
	pfc.precomp_for_mult(g2a);  // g2a is fixed, so precompute on it

	h=new G2[U]; //declare variables for the generators fo G2 and a pointer h.
	for (i=0;i<U;i++)
	{
		pfc.random(h[i]);
		pfc.precomp_for_mult(h[i]);  // precompute on h[.]
	}
	MSK=pfc.mult(g1,alpha);         //Compute the master secret key MSK=(g1)^a

// Encrypt

	m=find_m(Access);             //find the number of leaf nodes in access structure
	d=find_d(Access);			//find the number of columns in the access structure 	

	LSSS=new Big *[m];          //get a handler for LSSS martix
	for (i=0;i<m;i++)
		LSSS[i]=new Big[d+1]; // get memory for each row
	attr=new int [m];           //pointer for the attributes.

	make_LSSS(Access,m,d,LSSS,attr); // create LSSS matrix from Access structure
	
	cout << "LSSS matrix " << m << "x" << d << endl; //print the LSSS Matrix
	for (i=0;i<m;i++)
	{
		for (j=0;j<d;j++)                    //search for the nodes
			cout << LSSS[i][j] << " ";
		cout << "    " << (char)attr[i] << endl;     //print the  attribute at nodes.
	}
	cout << endl;    // go to next for next print

	mip->IOBASE=256;        //get handle on mip
	M=(char *)"  crypto";   // get the messaage value in this case the value of message is "crypto"
	cout << "Message to be encrypted=   " << M << endl; // print "Message to be encrypted" and the message value.
	mip->IOBASE=16;       ///get handle on mip

	Big *v=new Big [d];       //handler for the number of columns
	Big *lambda=new Big [m];   //lambda handler for the number of leaf nodes
	Big *r=new Big [m];         //r handler for the number of leaf nodes
	for (i=0;i<d;i++)             
		pfc.random(v[i]);
	s=v[0];
	for (i=0;i<m;i++)
	...
	CT=lxor(M,pfc.hash_to_aes_key(pfc.power(eg2g1a,s)));//   compute CT=Me(g,g)^(alpha,s)
	CD=pfc.mult(g2,s);                                  //compute g2^s
...
// keygen
	...
	pfc.random(t);                     //compute random t
	K=MSK+pfc.mult(g1a,t);             //compute k=(g1)^(alpha+at).
	L=pfc.mult(g1,t);              //compute  (ga)^t

	G2 *KX=new G2[U];              //handler KX for the group G2
	nS=0;
	while (auth[nS]!=0)   //compute the pairing
	{
		j=auth[nS++]-'A';
		KX[j]=pfc.mult(h[j],t);
...
// decrypt - and apply some optimisations e.g. e(A,B)*e(A,C) = e(A,B+C)
    ...
    int *rows=new int [m];   //pointer for number of leaf nodes
	Big *w=new Big [m];       

	if (!reduce_LSSS(order,m,d,LSSS,attr,auth,rows,w)) // check for the number of available attributes
	{
...
// combine rows which share same attribute
	...
	M=lxor(CT,pfc.hash_to_aes_key(mask));  //The decryption algorithm can then divide out mask value from C and obtain the message 
